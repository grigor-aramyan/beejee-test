...
class Dispatcher {
    ...
    public final function dispatch() {
        $chunks = explode('/', trim($_SERVER['REQUEST_URI'], '\/'));
        $selected_path = '/' . $chunks[count($chunks) - 1];
        $this->router->route(
            $_SERVER['REQUEST_METHOD'],
            $this->pathFromUri($selected_path),
            $_REQUEST);
    }
...
class Router {
    ...
    private function requestMatches($pattern, $path, $patternParams, &$params) {
        ...
        if (preg_match("/^{$pattern}$/i", $path, $matches)) {
            if (!$patternParams) return true;
            for ($i = 0; $i < count($patternParams); $i++) {
